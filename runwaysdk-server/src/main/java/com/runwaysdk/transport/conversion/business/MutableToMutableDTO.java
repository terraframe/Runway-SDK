...
public abstract class MutableToMutableDTO extends ComponentIFtoComponentDTOIF
{
  ...
  protected TypeMd createTypeMd()
  {
    if (this.convertMetaData())
    {
      ...
      return new TypeMd(this.getMdTypeIF().getDisplayLabel(locale), this.getMdTypeIF().getDescription(locale), this.getMdTypeIF().getId(), this.getMdTypeIF().isGenerateSource());
    }
    else
    {
      return new TypeMd();
