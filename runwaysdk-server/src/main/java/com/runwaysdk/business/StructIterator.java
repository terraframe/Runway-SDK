...
public class StructIterator<T> extends ComponentIterator<T>
{
  protected MdEntityDAOIF  rootMdEntityIF;

  public StructIterator(MdEntityDAOIF mdEntityIF, Map<String, ColumnInfo>  columnInfoMap, ResultSet resultSet)
  {
  ...
  public T next()
  {
    ...
    if (this.hasNext)
    {
      try
      {
        ...
        {
         struct = Struct.typeUnsafeStructFactory(structDAO);
        }
        else
        {
          struct = Struct.instantiate(structDAO);
        }
