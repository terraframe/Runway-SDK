 ...
 *
 *
 * Handles the mdRelationship tags so that the parent and child tags within them are always
 * processed before the other components, irrespective of the order of appearance of parent and child
 * in the XML document. In order to achieve this, it only processes the parent and child tag and caches all the events
 * related to any other tag. When it sees the end of the mdRelationship tags the cached events are dispatched to
 * a {@link MdRelationshipHandler}.
 *
 *
 */
public class MdRelationshipHandlerWithCaching extends MdRelationshipHandler
{
  ...
  public MdRelationshipHandlerWithCaching(Attributes attributes, XMLReader reader,
      XMLHandler previousHandler, ImportManager manager, String tagName)
  {
  ...
  public void endElement(String namespaceURI, String localName, String fullName) throws SAXException
  {
    ...
    if (localName.equals(XMLTags.MD_RELATIONSHIP_TAG) || localName.equals(XMLTags.MD_TREE_TAG)
        || localName.equals(XMLTags.MD_GRAPH_TAG))
    {
      ...
      //once the end of the mdRelationship tag has been encountered, dispatch the cached events
      ...
      for (SAXEvent saxEvent : saxEvents)
      {
        try
        {
          if (saxEvent instanceof StartSAXEvent)
          {
            ...
            super.startElement(startEvent.uri, startEvent.localName, startEvent.name,
