...
public class GraphDAO extends RelationshipDAO implements GraphDAOIF
{
  ...
  protected void validate()
  {
    if (this.isNew())
    {
      ...
      {
        List<RelationshipDAOIF> relationshipIFList = RelationshipDAOFactory.get(this.getParentId(), this.getChildId(), this.getType());

        if (relationshipIFList.size() >= 1)
        { 
          MdRelationshipDAOIF mdRelationshipIF = this.getMdRelationshipDAO();
      
          String error = "A relationship of type [" + mdRelationshipIF.definesType()
              + "] already exists between parent object with id [" + this.getParentId()
              + "] and child object with id [" + this.getChildId() + "]";
          DuplicateGraphPathException e = new DuplicateGraphPathException(error);
          e.init(mdRelationshipIF, this.getParentId(), this.getChildId());
          throw e;
        }
      }
    }
