...
public class ControllerJSGenerator extends TypeJSGenerator
{
  ...
  // FIXME generate as singleton and have static methods delegate to instance methods via getInstance()
  ...
  protected List<Declaration> getStaticMethods()
  {
    ...
    for(MdActionDAOIF action : actions)
    {
      ...
      if(params.size() == 1 && params.get(0).getParameterType().getType().equals(FormObjectInfo.CLASS))
      {
