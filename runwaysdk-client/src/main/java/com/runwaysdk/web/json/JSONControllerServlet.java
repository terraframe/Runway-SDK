...
public class JSONControllerServlet extends HttpServlet
{
  ...
  private String getOutputFromMethod(HttpServletRequest req, HttpServletResponse res, ClientSession clientSession)
  {
    try
    {
      ...
      // Check if we're invoking an MdFacade
      String controller = req.getParameter(MdFacadeInfo.JSON_ADAPTER_NAMESPACE);
      if (controller != null)
      {
        try
        {
          Class<?> clazz = LoaderDecorator.load(controller);
          Method facadeMethod = clazz.getMethod(method, String.class, Map.class);
          String json = (String) facadeMethod.invoke(null, sessionId, parameters);
          return json;
        }
