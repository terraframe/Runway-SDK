...
//      boolean isCountry = parents.size() == 1 && parents.get(0).getId().equals(Universal.getRoot().getId());
//      
//      if (isCountry) {
...
//    }
  ...
  /**
   * This MdMethod will apply the Universal dto and append it to parentId with relationshipType.
   * If parentId == RootUniversal then it will also create a corresponding GeoEntity.
   * 
   * @param dto
   * @param parentId
  ...
  private static TermAndRel doCreate(Universal child, String parentId, String relationshipType) {
    ...
    // Creating a Country Universal automatically creates a GeoEntity of the same name.
...
//   */
//  @AbortIfProblem
//  private static UniversalView createUniversalInternal(UniversalInput universalInput)
//  {
...
//
//  /**
//   * Returns this Universal as a UniversalView object. This will copy the values
...
//  }
  ...
  /**
   * Removing a link between a Universal may invalidate some GeoEntity dependencies. Do an Enforcement check here.
   */
...
//    {
//      ImmutableRootException exception = new ImmutableRootException("Cannot modify the root universal");
...
//  }
  ...
  public void removeLink(Term parent, String relationshipType)
  {
    ...
    // If we just invalidated the GeoEntity tree this enforcement will throw an exception and the transaction will rollback, undoing that remove link.
  ...
  private void enforceValidRemoveLink(Term parent, String relationshipType) {
    // Its possible that the link between these two universals may exist, but by a different relationship type.
    // If this link exists, then we're not deleting the link and the GeoEntites won't be invalidated, so we skip the check.
//    boolean skipCheck = false;
//    String[] possibleRels = TermUtil.getAllParentRelationships(parent.getId());
...
//        RelationshipQuery relq = new QueryFactory().relationshipQuery(possibleRel);
...
//    if (!skipCheck) {
      ...
      // 1) Get all GeoEntites that reference this universal or any of our descendants
      GeoEntityQuery query = new GeoEntityQuery(new QueryFactory());
      ...
      try
      {
        while (iter.hasNext())
        {
          // Check to ensure that this GeoEntity is still valid. A GeoEntity is valid iff its universal is a descendant of the parent geo's universal
