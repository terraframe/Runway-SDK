...
public class JSONRemoteAdapterGenerator extends AbstractCommonGenerator
{
  public JSONRemoteAdapterGenerator(MdFacadeDAOIF mdFacade)
  {
  ...
  {
    JSONRemoteAdapterGenerator generator = new JSONRemoteAdapterGenerator(mdFacadeIF);
    generator.go(false);
  }
  ...
  {
    return this.getSerialVersionUID("JSON_REMOTE_ADAPTOR", this.getSignature());
  }
  ...
  {
    getWriter().write("public interface " + getFileName() + " extends " + Remote.class.getName());
  ...
  {
    String interfaceName = this.getMdTypeDAOIF().getPackage() + "." + getGeneratedName(this.getMdTypeDAOIF());
    getWriter().writeLine("public static final String SERVICE_NAME = \"json:" + interfaceName + "\";");
  }
  ...
  {
    return "JSON" + mdFacade.getValue(MdFacadeInfo.NAME) + "RemoteAdapter";
  }
}
