...
//  static final ReentrantLock reentrant = new ReentrantLock();
   ...
   *
   * @param array The locations of the generatedClassPaths
   * @param parent The parent loader to delegate to for non Reloadable types
   */
...
//        throw new ConfigurationException(errMsg);
  ...
  /**
   * actualLoad breaks the standard delegation model for ClassLoaders. It reads the bytes
   * of the requested class, and if the class implements {@link Reloadable}, then it
   * loads the class <b>without</b> delegating up the loader hierarchy. If the target
   * class does not implement {@link Reloadable}, then this well delegate to the parent
   * classloader.
   *
  ...
  public Class<?> actualLoad(String name, boolean resolve) throws ClassNotFoundException
  {
    ...
    // Encompass everything in a try block so we can use finally to ensure that the lock is released
    try
    {
...
//    throw new ClassNotFoundException("Unable to find class " + name);
  ...
  /**
   * Loads an array from the base component up. If an array is loaded without its
   * componentType already loaded then an error occurs. Thus it loads from inside out.
   *
   * @param arrayType
   */
  private Class<?> loadArray(String arrayType)
  {
    // Keep track of what types of array we have (an array of Integers and an array of
