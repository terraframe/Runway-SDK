...
public class PermissionHandlerFactory implements HandlerFactoryIF
{
   ...
   * (non-Javadoc)
   * 
   * @see com.runwaysdk.dataaccess.io.dataDefinition.HandlerFactoryIF#getHandler
  ...
  public XMLHandler getHandler(String localName, Attributes attributes, XMLReader reader, XMLHandler handler, ImportManager manager)
  {
    ...
    {
      return new UserPermissionHandler(attributes, reader, handler, manager);
    }
    else if (localName.equals(XMLTags.ROLE_TAG))
    {
      return new RolePermissionHandler(attributes, reader, handler, manager);
    }
    else if (localName.equals(XMLTags.METHOD_TAG))
    {
      return new MethodPermissionHandler(attributes, reader, handler, manager);
    }
